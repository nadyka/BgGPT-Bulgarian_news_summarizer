...
def display_articles():
    ...
    if not todays_articles.empty:
        ...
        for i, row in enumerate(todays_articles.iterrows()):
            index, data = row
            with col1 if i % 2 == 0 else col2:  # Alternate between the two columns
                with st.expander(f"{data['title']}"):
                    st.write(f"Накратко: {data['summary']}")
                    st.write(f"Прочетете цялата статия: [link]({data['link']})")
    else:
...
#composite synthetic website key; the sequence of the first 10 characters. GUID- globally unique identifier; UII - unique identifier