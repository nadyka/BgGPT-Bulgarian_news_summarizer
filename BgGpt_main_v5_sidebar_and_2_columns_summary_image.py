...
def display_articles():
    ...
    selected_date = st.sidebar.date_input("Select a date / Изберете дата", datetime.date.today())
    if st.sidebar.button("Fetch News Now / Покажете новините сега"):
    ...
    if not todays_articles.empty:
        ...
        selected_article = st.sidebar.selectbox('Select an article / Изберете статия', todays_articles['title'])
        selected_article_data = todays_articles[todays_articles['title'] == selected_article].iloc[0]
        col1, col2 = st.columns(2)  # Create two columns
        with col1:  # Display the summary in the left column
            with st.expander(f"{selected_article_data['title']}"):
                st.write(f"Summary / Накратко: {selected_article_data['summary']}")
                st.write(f"Read the article / Прочетете цялата статия: [link]({selected_article_data['link']})")
        with col2:  # Display the image in the right column
            image_url = extract_image_url(selected_article_data['link'])
            if image_url:
                st.image(image_url)
    else:
        st.write("No articles found/ Не са намерени статии.")
